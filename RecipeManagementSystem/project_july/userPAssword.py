...
class password:
    def __init__(self,useremail):
        ...
        self.txt2 = Entry(self.frame, font=self.font, width=20)
        ...
        self.txt3 = Entry(self.frame, font=self.font, width=20)
        self.l3.grid(row=2, column=0, pady=20, padx=10)
        self.txt3.grid(row=2, column=1, pady=20, padx=10)

        self.btn = Button(self.root, text="Submit", width=20,  font=self.font, command=self.Check,
    ...
    def Check(self):
        ...
        conn = Connect()
        cr = conn.cursor()
        q = f"select email , password from user where email='{email}' and password='{password}'"
        cr.execute(q)
        result = cr.fetchall()
        if len(result) == 0:
